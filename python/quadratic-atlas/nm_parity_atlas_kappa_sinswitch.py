...
ESCAPE_RADIUS = 2.0  # standard quadratic escape threshold
...
# -----------------------------
# Parity diagnostics (sign–sin switching)
# x_{n+1} = sigma_n x_n^2 + c
# sigma_{n+1} = sign(sin(kappa * x_{n+1}))
# -----------------------------
def sinswitch_parity_diagnostics(C, B, kappa, max_iter=MAX_ITER, escape_radius=ESCAPE_RADIUS):
    x = B.copy()
    ...
    for n in range(max_iter):
        ...
        # record first flip iteration (store n when flip occurs the first time)
        ff = first_flip_iter[idx]
        ff_new = np.where(flips & (ff < 0), n, ff)
        first_flip_iter[idx] = ff_new

        # accumulate flips
        flip_count[idx] += flips.astype(np.int32)
...
for kappa in KAPPA_VALUES:
    data = sinswitch_parity_diagnostics(C, B, kappa)
