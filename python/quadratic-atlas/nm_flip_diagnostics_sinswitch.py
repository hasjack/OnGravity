...
ESCAPE_RADIUS = 2.0  # aligns with standard quadratic escape threshold
...
# -----------------------------
# Sign–sin helper: ensure sigma ∈ {±1} only
# Convention: sin(...) == 0 -> +1
# -----------------------------
def sign_sin_vec(kappa, x):
    return np.where(np.sin(kappa * x) >= 0.0, 1.0, -1.0)

# -----------------------------
# Orbit diagnostics (sign–sin switching)
# x_{n+1} = sigma_n x_n^2 + c
# sigma_{n+1} = sign(sin(kappa * x_{n+1}))
# -----------------------------
def nm_orbit_diagnostics_sinswitch(C, B, kappa, max_iter=MAX_ITER, escape_radius=ESCAPE_RADIUS):
    x = B.copy()
    sigma = sign_sin_vec(kappa, x)
    ...
    for n in range(max_iter):
        ...
        # occupancy before update (counts time spent in current sigma)
        plus_mask = s_a > 0
        ...
        # new sigma from sign(sin(kappa * x_next))
        s_next = sign_sin_vec(kappa, x_next)

        flips = (s_next != s_a)
        flip_count[idx[0][flips], idx[1][flips]] += 1
...
for kappa in KAPPA_VALUES:
    data = nm_orbit_diagnostics_sinswitch(C, B, kappa)
    ...
    axs[0, 0].set_title(f"Switching escape-time atlas\n$\\kappa={kappa}$")
